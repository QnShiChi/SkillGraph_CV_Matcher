...
backup-db:
	mkdir -p $(BACKUP_DIR)
	$(COMPOSE) exec -T postgres pg_dump -U $(POSTGRES_USER) -d $(POSTGRES_DB) --clean --if-exists --no-owner --no-privileges > $(BACKUP_FILE)
	@echo "Backup created: $(BACKUP_FILE)"

restore:
	test -n "$(BACKUP_FILE)"
	test -f "$(BACKUP_FILE)"
	case "$(BACKUP_FILE)" in *.sql) ;; *) echo "BACKUP_FILE must point to a .sql file"; exit 1;; esac
